...
- **Input_total_nutrients.csv** - total daily nutrient data to go with
  dietary data.
- **FooDB_polyphenol_classtax_3072.csv** - class taxonomy is derived
  from FooDB which uses ClassyFire, an automated chemical taxonomic
  classification application based on chemical structure

### OUTPUTS

- **Input_FooDB_polyphenol_content_class_by_recall.csv**, polyphenol
  class intakes by recall for each participant
- **Input_FooDB_polyphenol_content_class_by_subject.csv**, polyphenol
  class intakes for each participant, provided in wide format (classes
  as rows)
- **Input_FooDB_polyphenol_content_class_by_subject_wide.csv**,
  polyphenol class intakes for each participant, provided in wide format
  (classes as columns)
...
vroom::vroom_write(class_intakes_recall, "outputs/Input_FooDB_polyphenol_content_class_by_recall.csv")
```
...
vroom::vroom_write(class_intakes_subject,
                   "outputs/Input_FooDB_polyphenol_content_class_by_subject.csv")
```
...
vroom::vroom_write(class_intakes_subject_wide,
                   "outputs/Input_FooDB_polyphenol_content_class_by_subject_wide.csv")
```
